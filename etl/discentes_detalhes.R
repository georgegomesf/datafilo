...
for(arquivo in arquivos){
  ...
  if(!is.na(td)){
    td <- subset(td,area %like% "FILOSOFIA",select=c(AN_BASE,SG_UF_ENTIDADE_ENSINO,SG_ENTIDADE_ENSINO,CD_PROGRAMA_IES,NM_PROGRAMA_IES,NM_NIVEL_TITULACAO_DISCENTE,NM_SITUACAO_DISCENTE))
...
for(arquivo in arquivos){
  ...
  td <- fread(paste0(dir,arquivo), encoding = 'Latin-1')
  if(!is.na(td)){
    td <- subset(td,area %like% "FILOSOFIA",select=c(AN_BASE,SG_UF_PROGRAMA,SG_ENTIDADE_ENSINO,CD_PROGRAMA_IES,NM_PROGRAMA_IES,DS_GRAU_ACADEMICO_DISCENTE,NM_SITUACAO_DISCENTE))
...
write.csv2(view,"tabelas/discentes_detalhes.csv",row.names=F,na="")
